...
def install_log 
  ...
  force_mode = @installer.force
  ruby_block "validate cpan client logs for #{installed_module}" do
    block do
...
action :install do
  ...
  directory "/tmp/local-lib/ for #{new_resource.name}" do
    path '/tmp/local-lib/'
    owner user
    group group
  end

  directory "/tmp/local-lib/install for #{new_resource.name}" do
    path '/tmp/local-lib/install'
    owner user
    group group
  end
  ...
  cookbook_file "/tmp/local-lib/.modulebuildrc for #{new_resource.name}" do
    path '/tmp/local-lib/.modulebuildrc'
    cookbook 'cpan'
    action :create
...
def install_cpan_module args = { }
    ...
    if module_name != '.'
        ...
        bash "checking if module exists at CPAN - #{module_name}" do
            code <<-CODE
                #{local_lib_stack}
                perl -MCPAN -e '
    ...
    if @installer.version.nil? && module_name != '.' && @test_mode.nil? # not install if uptodate
        bash "installing cpan module #{module_name}" do
            code <<-CODE
                #{local_lib_stack}
                perl -MCPAN -e '
                my $m = CPAN::Shell->expand("#{cpan_type}","#{module_name}");
